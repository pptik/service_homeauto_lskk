...
namespace daemon_rmqservices_gpio
{
    ...
    public class ConsumeRabbitMQHostedService : BackgroundService
    {
        ...
        private void StatusConnection()
        {
           ...
           var lightTimeInMilliseconds = 2000;
            ...
            PingReply reply = testPing.Send("192.168.4.1",1000);
           ...
           private void StatusConnectionRMQ()
        {
           var pin = 27;
           var lightTimeInMilliseconds = 2000;
            Ping testPing = new Ping();
            PingReply reply = testPing.Send("192.168.4.2",1000);
            using (GpioController controller = new GpioController())
            {
                controller.OpenPin(pin, PinMode.Output);
                if (reply.Status == IPStatus.Success ){
